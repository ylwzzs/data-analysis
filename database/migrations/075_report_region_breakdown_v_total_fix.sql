-- 075_report_region_breakdown_v_total_fix.sql
-- 修复：总部目标(target_level='total')也能查到下钻数据
-- 原视图只取 breakdown_level='store' 的子目标，导致总部目标(id=22)查询返回空
-- 修复：target_base 同时取 total 和 store 两种目标；
--   total 目标的目标值来自自身 target_metric_values；
--   store 子目标的目标值来自子目标的 target_metric_values（按 branch_num 匹配）
...
-- 销售目标值：total 目标取自身，store 子目标取自身
sale_targets AS (
...
-- 门店层基础数据
-- 关键变化：total 目标的目标值分配到所有门店（按总目标均分）
--           store 子目标的目标值取自身
store_level AS (
  -- total 目标：目标值均分到所有考核门店
  SELECT
    tb.target_id,
    'store' AS level,
...
DO $$ BEGIN RAISE NOTICE 'Migration 075 completed: report_region_breakdown_v fixed for total targets'; END $$;

...
export default function BreakdownPage() {
  ...
  const load = async () => {
    ...
    if (j.mode === 'hq') {
      ...
      const savedM: Record<string, any> = Object.fromEntries((j.rows || []).map((x: any) => [x.category, x.metrics]));
      setHqGrid(Object.fromEntries(HQ_CATEGORIES.map(c => [c, Object.fromEntries(HQ_METRICS.map(m => [m, savedM[c]?.[m] ?? '']))])));
    } else {
      setRows(j.rows || []);
